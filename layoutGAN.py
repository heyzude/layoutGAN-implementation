...
class layoutGAN(object):
    ...
    def generator(self, reuse=False):
        with tf.variable_scope("generator", reuse=reuse):
            ...
            g_d_p_w = tf.get_variable('g_d_p_w', shape=[self.num_classes, self.num_classes], initializer=self.weight_init)
            g_d_theta_w = tf.get_variable('g_d_theta_w', shape=[self.num_geopara, self.num_geopara], initializer=self.weight_init)
            ...
            g_sa_hi_w = [tf.get_variable('g_sa_hi_w' + str(i), shape=[k, k], initializer=self.weight_init) for i in range(4)]
            g_sa_hj_w = [tf.get_variable('g_sa_hj_w' + str(i), shape=[k, k], initializer=self.weight_init) for i in range(4)]
            g_sa_u_w = [tf.get_variable('g_sa_u_w' + str(i), shape=[k, k], initializer=self.weight_init) for i in range(4)]
            g_sa_outer_w = [tf.get_variable('g_sa_outer_w' + str(i), shape=[k, k], initializer=self.weight_init) for i in range(4)]
            ...
            for n in range(3):  # total 4 layer -> forward for 3 steps.
                for i in range(self.num_elements):
                    ...
                    relation_layer[n+1][i] = tf.reshape( tf.matmul(sigma*float(1./self.num_elements), g_sa_outer_w[n]) + tf.reshape(relation_layer[n][i], shape=[1,k]), shape=[k] )  # residual addtion   # have to fix this with tf.scatter_nd
    ...
    def discriminator_rb(self, p, theta, reuse=False):  # relation-based diecriminator
        with tf.variable_scope("discriminator_rb", reuse=reuse):
            ...
            d_d_b3 = tf.get_variable('d_d_b3', shape=[1], initializer=self.weight_init)   # true, or false? 0.5 threshold by sigmoid

            d_d_p_w = tf.get_variable('d_d_p_w', shape=[self.num_classes, self.num_classes], initializer=self.weight_init)
            d_d_theta_w = tf.get_variable('d_d_theta_w', shape=[self.num_geopara, self.num_geopara], initializer=self.weight_init)
            ...
            d_sa_hi_w = [tf.get_variable('d_sa_hi_w' + str(i), shape=[k, k], initializer=self.weight_init) for i in range(2)]
            d_sa_hj_w = [tf.get_variable('d_sa_hj_w' + str(i), shape=[k, k], initializer=self.weight_init) for i in range(2)]
            d_sa_u_w = [tf.get_variable('d_sa_u_w' + str(i), shape=[k, k], initializer=self.weight_init) for i in range(2)]
            d_sa_outer_w = [tf.get_variable('d_sa_outer_w' + str(i), shape=[k, k], initializer=self.weight_init) for i in range(2)]
            ...
            for n in range(1):  # total 2 layer -> forward for 1 steps.
                for i in range(self.num_elements):
                    ...
                    relation_layer[n+1][i] = tf.reshape( tf.matmul(sigma*float(1./self.num_elements), d_sa_outer_w[n]) ) # no shortcut connection   # have to fix this with tf.scatter_nd
        ...
        for x in self.geo_range[0]:
            ...
            for y in self.geo_range[1]:
                ...
                for c in range(self.num_classes):
                    ...
                    I[x][y][c] = tf.math.reduce_max( np.array([ tf.nn.relu(1. - tf.math.abs(x - theta[i][0])) * tf.nn.relu(1. - tf.math.abs(y - theta[i][1])) * p[i][c] for i in range(self.num_elements) ]), axis=0)
        ...
        for x in self.geo_range[0]:
            ...
            for y in self.geo_range[1]:
                ...
                for c in range(self.num_classes):
                    ...
                    tf.math.reduce_max( [tf.math.reduce_max(np.array([ tf.nn.relu(1. - tf.math.abs(x - theta[i][0])) * tf.reduce_min([tf.nn.relu(y-theta[i][1]), 1], axis=0) * tf.reduce_min([tf.nn.relu(theta[i][3] - y), 1], axis=0),
                                                  tf.nn.relu(1. - tf.math.abs(x - theta[i][2])) * tf.reduce_min(
                                                      [tf.nn.relu(y - theta[i][1]), 1], axis=0) * tf.reduce_min(
                                                      [tf.nn.relu(theta[i][3] - y), 1], axis=0),
                                                  tf.nn.relu(1. - tf.math.abs(y - theta[i][1])) * tf.reduce_min(
                                                      [tf.nn.relu(x - theta[i][0]), 1], axis=0) * tf.reduce_min(
                                                      [tf.nn.relu(theta[i][2] - x), 1], axis=0),
                                                  tf.nn.relu(1. - tf.math.abs(y - theta[i][3])) * tf.reduce_min(
                                                      [tf.nn.relu(x - theta[i][0]), 1], axis=0) * tf.reduce_min(
                                                      [tf.nn.relu(theta[i][2] - x), 1], axis=0) ]  )) * p[i][c] for i in range(self.num_elements)]    , axis=0)
    ...
    def discriminator_wr(self, p, theta, geo_type='point', reuse=False):   # wireframe rendering discriminator
        with tf.variable_scope("discriminator_wr", reuse=reuse):
            ...
            flat = tf.reshape(pool3, [-1, ( math.ceil(math.ceil(math.ceil(len(rendered) / 2.) / 2.) / 2.)  ) * ( math.ceil(math.ceil(math.ceil(len(rendered[0]) / 2.) / 2.) / 2.)  ) * 16])
